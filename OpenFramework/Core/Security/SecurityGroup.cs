...
namespace OpenFrameworkV3.Core.Security
{
    ...
    public partial class SecurityGroup
    {
        ...
        public static ReadOnlyCollection<SecurityGroup> ByUserId(long userId, long companyId, string instanceName)
        {
            var cns = Persistence.ConnectionString(instanceName);
            var res = new List<SecurityGroup>();
            ...
            if (!string.IsNullOrEmpty(cns))
            {
                 ...
                 *   @CompanyId bigint */
                using (var cmd = new SqlCommand("Core_SecurityGroup_ByUserId"))
                {
                    using (var cnn = new SqlConnection(cns))
                    {
                        cmd.Connection = cnn;
                        cmd.CommandType = CommandType.StoredProcedure;
                        cmd.Parameters.Add(DataParameter.Input("@SecurityUserId", userId));
                        cmd.Parameters.Add(DataParameter.Input("@CompanyId", companyId));
                        ...
                        try
                        {
                            ...
                            using (var rdr = cmd.ExecuteReader())
                            {
                                while (rdr.Read())
                                {
                                    res.Add(new SecurityGroup
                                    {
                                        Id = rdr.GetInt64(ColumnsGroupGet.Id),
                                        CompanyId = rdr.GetInt64(ColumnsGroupGet.CompanyId),
            ...
            return new ReadOnlyCollection<SecurityGroup>(res);
        }

        /// <summary>Gets all groups</summary>
        public static ReadOnlyCollection<SecurityGroup> All(long companyId, string instanceName)
        {
            var cns = Persistence.ConnectionString(instanceName);
            var res = new List<SecurityGroup>();
            ...
            if (!string.IsNullOrEmpty(cns))
            {
                using (var cmd = new SqlCommand("Core_SecurityGroup_GetAll"))
                {
                    using (var cnn = new SqlConnection(cns))
                    {
                        cmd.Connection = cnn;
        ...
        /// <summary>Gets or company identifier</summary>
        ...
        public static SecurityGroup ById(long securityGroupId, long companyId, string instanceName)
        {
            var res = SecurityGroup.Empty;
            var cns = Persistence.ConnectionString(instanceName);
            if (!string.IsNullOrEmpty(cns))
            {
                        ...
                        try
                        {
                            ...
                            using (var rdr = cmd.ExecuteReader())
                            {
                                while (rdr.Read())
                                {
                                    ...
                                    res.CompanyId = rdr.GetInt64(ColumnsGroupGet.CompanyId);
        ...
        public static string JsonListSimple(ReadOnlyCollection<SecurityGroup> list)
        {
            var res = new StringBuilder("[");
            bool first = true;
        ...
        /// <param name="list">Groups list</param>
        /// <returns>A list of groups in JSON format</returns>
        public static string JsonList(ReadOnlyCollection<SecurityGroup> list)
        {
            var res = new StringBuilder("[");
            bool first = true;
            foreach (var group in list)
